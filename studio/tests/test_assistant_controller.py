...
# ── diagnostics passthrough (reuses the real napari_app.advisor engine) ───────
...
def test_refresh_status_ollama_reports_reachable_and_models(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("napari_app.advisor.ollama_models", lambda: ["a", "b"])
    results = []
...
def test_send_async_ollama_streams_and_builds_system_prompt(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("napari_app.advisor.ollama_chat", fake_ollama_chat)
