...
touches the network) behind three interchangeable chat backends:
  ...
  * **"offline"** — the deterministic diagnostic engine
    (``napari_app.advisor.diagnose``). Always available, no model, no
  ...
  * **"ollama"** — a locally running Ollama server. Reuses
    ``napari_app.advisor``'s existing Ollama bridge verbatim (model
  ...
  * **"custom"** — any OpenAI-compatible HTTP endpoint: local (LM Studio,
    ...
    idiom as ``napari_app.advisor.ollama_chat``.
...
def custom_api_chat(base_url: str, api_key: str, model: str, messages: list[dict[str, str]],
    ...
    ``napari_app.advisor.ollama_chat``, same contract (streams tokens via
...
class AssistantController:
    ...
    # ── diagnostics (reuses napari_app.advisor's engine, read-only) ─────────
    @staticmethod
    ...
    # ── Ollama model management (reuses napari_app.advisor verbatim) ────────
    @staticmethod
